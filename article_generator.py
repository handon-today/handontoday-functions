...
def run_pipeline_from_data(articles, test_mode=False, max_pairs=4, recent_hours=24, save_files=False):
    ...
    Args:
        articles: 기사 리스트 (korea + overseas 합친 것)
        test_mode: True면 1쌍만 생성
        max_pairs: 최대 짝 수
        recent_hours: 최근 N시간 이내 기사만
        save_files: True면 로컬 파일에도 저장 (테스트용), False면 결과만 반환
    ...
    # Cloud Functions에서는 로컬 파일 저장 안 함 (Cloud Storage에 main.py가 따로 저장)
